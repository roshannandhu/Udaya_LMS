...
15. **Student ID = `students.student_code` (DB column), labelled "Student ID" in the UI.** Do NOT confuse it with `student_id`, which means the UUID throughout `main.py`. Format is glued, no separators: `{INSTITUTION}{YYYY}{STD}{SEQ}` e.g. `UDAYA202510001` — institution auto-derived from the branding name (`_institution_prefix`), admission year, standard number, then a running sequence per `{INSTITUTION}{YYYY}{STD}` prefix. Generated by `generate_student_code()` / persisted by `assign_student_code()` (post-insert UPDATE, like `plain_password`, so a missing column never blocks student creation) on all three create paths (`create_student_admin`, `bulk_import_students`, `approve_join_request`). A partial unique index (`idx_students_student_code … WHERE student_code IS NOT NULL`) is the hard uniqueness guarantee. Students can log in with it: `login()` resolves a no-`@` identifier as a student code first, then falls back to phone. Backfill existing students via `POST /api/admin/backfill-student-codes` (teacher-only, idempotent; Settings → Students button).
