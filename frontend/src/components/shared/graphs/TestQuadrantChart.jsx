    ...
    <div className="w-full h-full min-h-[220px] pt-4 pb-2 pr-4">
        ...
        <ScatterChart margin={{ top: 20, right: 30, left: 10, bottom: 30 }}>
          ...
          <Scatter name="Tests" data={data}>
            {data.map((entry, index) => {
              ...
              let color = '#FCA5A5'; // Slow & Low (bottom left)
              if (entry.score >= 50 && entry.time <= 45) color = '#67E8F9'; // Fast & High
              else if (entry.score >= 50 && entry.time > 45) color = '#A78BFA'; // Slow & High
              else if (entry.score < 50 && entry.time <= 45) color = '#FDE047'; // Fast & Low
      ...
      <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
        <div className="w-full h-full relative opacity-20 text-[9px] font-bold uppercase tracking-wider">
          <span className="absolute top-4 left-6 text-[#A78BFA]">Methodical</span>
          <span className="absolute top-4 right-6 text-[#67E8F9]">Mastered</span>
          <span className="absolute bottom-6 left-6 text-[#FCA5A5]">Struggling</span>
          <span className="absolute bottom-6 right-6 text-[#FDE047]">Rushing</span>
        </div>
      </div>
    </div>
  );
